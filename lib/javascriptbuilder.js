...
JavascriptBuilder.prototype.toString = function (init) {
    ...
    this.expectations.forEach(function (expectation) {
        mockFunction += this._buildSyncFunctionForExpectation(expectation);
    }, this);
...
JavascriptBuilder.prototype._buildSyncFunctionForExpectation = function (expectation) {
    var mockFunction = "window.httpBackendContext." + this.buildRequestmethod(expectation, 'expect') + ";";
    return mockFunction;
};

JavascriptBuilder.prototype.verifyNoOutstandingExpectation = function () {
    var mockFunction = "if (window && window.httpBackendContext && window.httpBackendContext.verifyNoOutstandingExpectation){" +
        "window.httpBackendContext.verifyNoOutstandingExpectation();" +
        "}";
    return mockFunction;
};

JavascriptBuilder.prototype.buildRequestmethod = function (request, prefix) {
    ...
    return prefix + request.method + "(" + url + ")";
};
